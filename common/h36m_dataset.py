...
class Human36mDatasetImages:
    def __init__(self, path, subjects,  extract=False, load_images=False):
        ...
        if extract:
            for s in self.subjects:
                from_path = os.path.join(self.path, s, 'Videos_' + s + '.zip')
                to_path = os.path.join(self.path, s, 'Videos_' + s)
                print('Extracting ' + from_path)
                extract_archive(from_path, to_path)

        self.filenames = []
        for s in self.subjects:
            file_list = glob(os.path.join(self.path, s, 'Videos_' + s) + '/*.mp4')
            assert len(file_list) == 120, "Expected 30 files for subject " + s + ", got " + str(len(file_list))
            for f in file_list:
                action = os.path.splitext(os.path.splitext(os.path.basename(f))[0])[0]

                if s == 'S11' and action == 'Directions':
