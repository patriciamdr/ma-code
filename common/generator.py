...
class ChunkedGenerator:
    def __init__(self, batch_size, cameras, poses_3d, poses_2d_keypoints, poses_scores, poses_vis, poses_2d_gt, chunk_length=1, pad=0, causal_shift=0,
        ...
        assert poses_scores is None or len(poses_scores) == len(poses_2d_keypoints)
        assert poses_2d_gt is None or len(poses_2d_gt) == len(poses_2d_keypoints)
        assert poses_vis is None or len(poses_vis) == len(poses_2d_keypoints)
        ...
        if poses_scores is not None:
            self.batch_scores = np.empty(
                (batch_size, chunk_length + 2 * pad, poses_scores[key].shape[-2], poses_scores[key].shape[-1]))
        ...
        if poses_vis is not None:
            example_key = next(iter(poses_vis))
            self.batch_vis = np.empty(
                (batch_size, chunk_length + 2 * pad, poses_vis[example_key].shape[-2], poses_vis[example_key].shape[-1]))
        ...
        self.poses_scores = poses_scores
        self.poses_vis = poses_vis
    ...
    def augment_batch(self, seq_name, start_3d, end_3d, flip, reverse):
        ...
        if self.poses_scores is not None:
            self.batch_scores = self.augment_seq(self.batch_scores, self.poses_scores, seq_name, start_3d, end_3d, flip, reverse,
                                                 self.kps_left, self.kps_right)
