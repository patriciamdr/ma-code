...
def run(args):
    ...
    for joint_idx in range(0, 17):
        ...
        with open(f'part_{filename}', 'w+') as outfile:
            ...
            if args.spatial_module:
                subprocess.call(frame_specific_command + ['--spatial_module'], stdout=outfile)
            else:
                subprocess.call(frame_specific_command, stdout=outfile)

        # if not args.consecutive_frames:
        #     # Occlude whole sequence
        #     with open(f'full_{filename}', 'w+') as outfile:
        #         frame_specific_command = joint_specific_command + ['--num_occluded_f', str(frames)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--spatial_module', action='store_true')
