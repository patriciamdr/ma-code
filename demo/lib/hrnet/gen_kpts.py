...
def gen_video_kpts(video, j, det_dim=416, num_peroson=1, gen_output=False, use_gt_bbox=False, gt_bboxs=None):
    def read_video():
        ret, frame = cap.read()
    ...
    def read_list():
        ...
        gt_bbox = gt_bboxs[ii]
        gt_score = 1
        return True, frame, gt_bbox, gt_score
    ...
    for ii in range(video_length):
        ret, frame, gt_bbox, gt_score = read()
        if not ret:
            continue
        ...
        with torch.no_grad():
            ...
            inputs, origin_img, center, scale = PreProcess(frame, track_bboxs, cfg.MODEL.IMAGE_SIZE, num_peroson)
            ...
            # compute coordinate
            preds, maxvals = get_final_preds(cfg.TEST.POST_PROCESS, output.clone().cpu().numpy(), np.asarray(center), np.asarray(scale))
