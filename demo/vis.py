...
def show2Dpose(kps, img, n_joints=17, vis_mask=None):
    if n_joints == 17:
    ...
    for j,c in enumerate(connections):
        ...
        if vis_mask is not None:
            start_color = end_color = (0, 255, 0)
            if not vis_mask[c[0]]:
                cv2.circle(img, (start[0], start[1]), thickness=-1, color=start_color, radius=5)
            if not vis_mask[c[1]]:
                cv2.circle(img, (end[0], end[1]), thickness=-1, color=end_color, radius=5)
        # else:
        #     start_color = end_color = (0, 255, 0)
        #     cv2.circle(img, (start[0], start[1]), thickness=-1, color=start_color, radius=5)
        #     cv2.circle(img, (end[0], end[1]), thickness=-1, color=end_color, radius=5)
...
def show3Dpose(vals, ax, n_joints=17, radius=(0.8, 0.8)):
    ...
    if n_joints == 17:
        ...
        I = np.array([0, 0, 1, 4, 2, 5, 0, 7, 8, 8, 9, 10, 12, 13])
        J = np.array([1, 4, 2, 5, 3, 6, 7, 8, 9, 12, 10, 11, 13, 14])
    ...
    ax.set_xlim3d([-RADIUS[0], RADIUS[1]])
    ax.set_ylim3d([-RADIUS[0], RADIUS[1]])
    ax.set_aspect('auto')
