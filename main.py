...
from model.strided_vis_graformer_gt import Model as StridedVisGraformerGT
...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        if opt.dataset == 'unrealcv':
            batch_cam, gt_3D, gt_2D, vis, input_2D, inputs_scores, extra = data
            video_id, cam_ind, action = extra
            [input_2D, inputs_scores, gt_3D, batch_cam, vis] = get_variable(split, [input_2D, inputs_scores, gt_3D, batch_cam, vis])
        else:
            batch_cam, gt_3D, gt_2D, vis, input_2D, inputs_2D_score, dist, scale, bb_box, extra = data
            action, subject, cam_ind = extra
            [input_2D, vis, gt_3D, gt_2D, batch_cam, scale, bb_box] = get_variable(split, [input_2D, vis, gt_3D, gt_2D, batch_cam, scale, bb_box])
        ...
        if split == 'train':
            if opt.use_visibility:
                output_3D, output_3D_VTE, output_vis, _, _, _, _ = model_trans(input_2D)
            else:
                output_3D, output_3D_VTE, _, _, _, _ = model_trans(input_2D)
        else:
            if opt.use_visibility:
        ...
        if split == 'train':
            if opt.refine:
                ...
                if opt.use_visibility:
                    loss_vis = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor(opt.pos_weight)).cuda()(output_vis, vis)
                    loss_all['loss_vis'].update(loss_vis.detach().cpu().numpy() * N, N)
                    loss = opt.pose_weight_factor * (loss_VTE + loss_single) + loss_vis
                else:
            ...
            if opt.refine:
                ...
                # error_sum_joints = test_calculation_joints_mpjpe(output_3D, out_target, action, error_sum_joints, dist)

            if opt.use_visibility:
                action_error_sum_vis_binary_class_metrics = \
                    test_calculation_binary_class_metrics(output_vis, vis, action, action_error_sum_vis_binary_class_metrics, opt.dataset)

                action_error_sum_vis_acc = test_calculation_acc(output_vis, vis, action, action_error_sum_vis_acc, opt.dataset)

    if split == 'train':
        if opt.use_visibility:
            return loss_all['loss'].avg, loss_all['loss_single'].avg, loss_all['loss_VTE'].avg, loss_all['loss_vis'].avg
        else:
        ...
        if opt.use_visibility:
            ...
            return p1, p2, acc, ap, npv, tnr, tpr
        else:
            return p1, p2
...
def input_augmentation(input_2D, model_trans):
    ...
    output_3D_non_flip, output_3D_non_flip_VTE, _, _, _, _= model_trans(input_2D_non_flip)
    output_3D_flip, output_3D_flip_VTE, _, _, _ , _ = model_trans(input_2D_flip)
...
if __name__ == '__main__':
    ...
    model = {}
    if opt.spatial_module:
        if opt.use_visibility_gt:
            model_trans = StridedVisGraformerGT(opt).cuda()
        elif opt.use_visibility:
            model_trans = StridedVisGraformer(opt).cuda()
        else:
            model_trans = StridedGraformer(opt).cuda()
    else:
        model_trans = Model(opt).cuda()
    ...
    if opt.use_visibility:
        params = [{"params": model['trans'].Transformer.parameters(), "lr": opt.spatial_module_lr},
                {"params": model['trans'].reduce.parameters()},
                {"params": model['trans'].Transformer_reduce.parameters()},
                {"params": model['trans'].Transformer_full.parameters()},
                {"params": model['trans'].visibility_class_head.parameters()},
                {"params": model['trans'].fcn.parameters()},
                {"params": model['trans'].head.parameters()},
                {"params": model['refine'].parameters(), "lr": opt.lr_refine}]
        if opt.extra_pose_hidden:
            params += [{"params": model['trans'].extra_pose_hidden.parameters()}]
        if opt.extra_vis_hidden:
            params += [{"params": model['trans'].extra_vis_hidden.parameters()}]
        optimizer_all = optim.Adam(params, lr=opt.lr, amsgrad=True)
    else:
    ...
    for epoch in range(epoch_start, opt.nepoch):
        ...
        if opt.train:
            ...
            if opt.use_visibility:
                loss, loss_single, loss_VTE, loss_vis = train(opt, actions, train_dataloader, model, optimizer_all, epoch)
            else:
        ...
        if opt.use_visibility:
            p1, p2, acc, ap, npv, tnr, tpr = val(opt, actions, test_dataloader, model)
        else:
        ...
        if not opt.train:
            if opt.use_visibility:
                print('p1: %.2f, acc: %.4f, ap: %.4f, npv: %.4f, tnr: %.4f, tpr: %.4f' % (p1, acc, ap, npv, tnr, tpr))
            else:
                print('p1: %.2f, p2: %.2f' % (p1, p2))
            break
        else:
            if opt.use_visibility:
                logging.info('epoch: %d, lr: %.7f, loss: %.4f, loss_single: %.4f, loss_VTE: %.4f, loss_vis: %.4f, p1: %.2f, p2: %.2f, '\
                        'acc: %.4f, ap: %.4f, npv: %.4f, tnr: %.4f, tpr: %.4f' % (
                    epoch, lr, loss, loss_single, loss_VTE, loss_vis, p1, p2, acc, ap, npv, tnr, tpr))
                print('e: %d, lr: %.7f, loss: %.4f, loss_single: %.4f, loss_VTE: %.4f, loss_vis: %.4f, p1: %.2f, p2: %.2f, '\
                        'acc: %.4f, ap: %.4f, npv: %.4f, tnr: %.4f, tpr: %.4f' % (
                    epoch, lr, loss, loss_single, loss_VTE, loss_vis, p1, p2, acc, ap, npv, tnr, tpr))
            else:
